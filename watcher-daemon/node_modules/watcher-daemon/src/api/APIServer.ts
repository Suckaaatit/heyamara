...
import { validateCompiledRule } from '../rules/RuleValidator';
  ...
  private setupRoutes(): void {
    ...
    this.app.post('/rules/compile', async (req: Request, res: Response) => {
      try {
        ...
        const validation = validateCompiledRule(compiled.rule);
        if (!validation.valid) {
          res.status(400).json({
    ...
    this.app.post('/rules', async (req: Request, res: Response) => {
      try {
        const { name, description, condition } = req.body as Partial<Rule> & { condition?: string };
        const errors: Record<string, string> = {};
    ...
    this.app.get('/matches', (_req: Request, res: Response) => {
      try {
        const matches = this.ruleEngine.getRecentMatches();
