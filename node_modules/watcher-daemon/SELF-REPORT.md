...
|---|---|---|
| File watching | ✅ Working | Recursive watch with debounce |
| Normalized events | ✅ Working | `created/modified/deleted` |
| Rule compilation | ✅ Working | LLM used only on rule creation (tinyllama + JSON repair) |
| Deterministic evaluation | ✅ Working | No LLM in evaluation |
| Count-based rules | ✅ Working | `threshold` rules with window + count |
| Pattern rules | ✅ Working | pathIncludes/extensions/eventTypes |
| Rule persistence | ✅ Working | JSON file in `data/rules.db` |
| Notifications | ✅ Working | Console + OS |
| Logging | ✅ Working | Structured winston logs |
| Local API | ✅ Working | /health, /rules, /matches, /report |
| Compile preview | ✅ Working | /rules/compile |
| Rule validation | ✅ Working | Rejects broad or unsafe rules |
| Optional local UI | ✅ Working | /ui for quick visibility; API + logs remain primary |

## 3. What Breaks / Limitations

| Issue | Severity | Description |
|---|---|---|
| JSON store concurrency | 🟡 Medium | No file-locking if multiple processes write |
| Match history | 🟡 Medium | Recent matches are in-memory only |
| Threshold windows | 🟡 Medium | In-memory counters reset on restart |
| LLM availability | 🟢 Low | Rule creation fails if Ollama is down |
| Rule expressiveness | 🟢 Low | Only pattern + threshold rules (by design) |
| Large models on low RAM | 🟡 Medium | `llama2` requires ~5.5GB RAM; on 8GB system use `tinyllama` |
...
- Single watch root: Simplifies configuration and aligns with the requirement to “monitor a directory.”
- No clustering: Explicitly out of scope for this challenge.

## 3.2 Runtime Notes (This Machine)

- Ollama model: `tinyllama` (fits in RAM)
- JSON reliability: forced `format: json` and JSON repair in the compiler
- Output is headless: API + logs + notifications (no UI)

## 4. Token Usage & Cost

LLM usage occurs **only on rule creation**.

- Each rule compile: ~200–800 tokens (prompt + response)
- Local Ollama cost: $0
