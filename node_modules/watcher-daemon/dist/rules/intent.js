...
exports.extractIntent = void 0;
const EVENT_ORDER = ['created', 'modified', 'deleted'];
...
function parseThresholdCount(text) {
    const patterns = [
...
function extractIntent(condition) {
    const text = condition.toLowerCase();
    const eventTypes = new Set();
    const extensions = new Set();
    const pathIncludes = new Set();
    ...
    const phraseMatches = text.matchAll(/\b(in|from|under|inside|within)\s+(?:the|a|an)?\s*([a-z0-9_.-]+)\b/gi);
